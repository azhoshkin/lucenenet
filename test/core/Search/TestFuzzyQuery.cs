...
namespace Lucene.Net.Search
{
    ...
    public class TestFuzzyQuery:LuceneTestCase
    {
        ...
        public virtual void  TestFuzzinessLong()
        {
            ...
            query = new FuzzyQuery(new Term("field", "aaaaccc"), FuzzyQuery.defaultMaxEdits, 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);
            Assert.AreEqual(searcher.Doc(hits[0].Doc).Get("field"), ("aaaaaaa"));

            // now with prefix
            query = new FuzzyQuery(new Term("field", "aaaaccc"), FuzzyQuery.defaultMaxEdits, 1);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);
            Assert.AreEqual(searcher.Doc(hits[0].Doc).Get("field"), ("aaaaaaa"));
            query = new FuzzyQuery(new Term("field", "aaaaccc"), FuzzyQuery.defaultMaxEdits, 4);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);
            Assert.AreEqual(searcher.Doc(hits[0].Doc).Get("field"), ("aaaaaaa"));
            query = new FuzzyQuery(new Term("field", "aaaaccc"), FuzzyQuery.defaultMaxEdits, 5);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);

            // no match, more than half of the characters is wrong:
            query = new FuzzyQuery(new Term("field", "aaacccc"), FuzzyQuery.defaultMaxEdits, 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);

            // now with prefix
            query = new FuzzyQuery(new Term("field", "aaacccc"), FuzzyQuery.defaultMaxEdits, 2);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);

            // "student" and "stellent" are indeed similar to "segment" by default:
            query = new FuzzyQuery(new Term("field", "student"), FuzzyQuery.defaultMaxEdits, 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);
            query = new FuzzyQuery(new Term("field", "stellent"), FuzzyQuery.defaultMaxEdits, 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);

            // now with prefix
            query = new FuzzyQuery(new Term("field", "student"), FuzzyQuery.defaultMaxEdits, 1);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);
            query = new FuzzyQuery(new Term("field", "stellent"), FuzzyQuery.defaultMaxEdits, 1);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);
            query = new FuzzyQuery(new Term("field", "student"), FuzzyQuery.defaultMaxEdits, 2);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);
            query = new FuzzyQuery(new Term("field", "stellent"), FuzzyQuery.defaultMaxEdits, 2);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);

            // "student" doesn't match anymore thanks to increased minimum similarity:
            query = new FuzzyQuery(new Term("field", "student"), 1, 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);
        ...
        public virtual void  TestTokenLengthOpt()
        {
            ...
            query = new FuzzyQuery(new Term("field", "1234569"), 2);
            ScoreDoc[] hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);

            // 10 chars, so no optimization
            query = new FuzzyQuery(new Term("field", "1234567891"), 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);

            // over 10 chars, so no optimization
            query = new FuzzyQuery(new Term("field", "12345678911"), 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(1, hits.Length);

            // over 10 chars, no match
            query = new FuzzyQuery(new Term("field", "sdfsdfsdfsdf"), 0);
            hits = searcher.Search(query, null, 1000).ScoreDocs;
            Assert.AreEqual(0, hits.Length);
        }
