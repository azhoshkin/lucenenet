...
namespace Lucene.Net.Search
{
    ...
    public sealed class FuzzyTermEnum:FilteredTermEnum
    {
        ...
        */
        ...
        /// </seealso>
        public FuzzyTermEnum(IndexReader reader, Term term):this(reader, term, FuzzyQuery.defaultMaxEdits, FuzzyQuery.defaultPrefixLength)
        {
        }
        ...
        public FuzzyTermEnum(IndexReader reader, Term term, float minSimilarity, int prefixLength):base()
        {
            ...
            if ((double)minSimilarity >= 1.0 && (double)minSimilarity != (double)(int)minSimilarity)
                throw new System.ArgumentException("fractional edit distances are not allowed");
            if ((double)minSimilarity < 0.0)
            ...
            // convert max edits to min similarity
            this.minimumSimilarity = minSimilarity >= 2.0f
                ? 0.5f
                : minSimilarity >= 1.0f
                    ? 0.75f
                    : minSimilarity < float.Epsilon
                        ? 0.99f
                        : minSimilarity;
